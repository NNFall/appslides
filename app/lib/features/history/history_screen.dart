        ...
        return ListView(
          ...
          children: [
            SectionCard(
              title: 'Local history',
              subtitle:
                  'Request and job history is stored locally on the device and survives app restart.',
            ...
            SectionCard(
              ...
              subtitle: savedEntries.isEmpty
                  ? 'After saving PPTX, PDF or DOCX, local result copies will appear here.'
                  : 'Files are stored in the app sandbox and are available without another backend request.',
            ),
            const SizedBox(height: 16),
            if (restoringSavedFiles)
              const SectionCard(
                ...
                subtitle:
                    'The app is restoring the list of locally saved results.',
                child: Padding(
              ...
              const SectionCard(
                ...
                subtitle:
                    'Save a result from Presentation or Converter and it will appear here.',
              )
            else
              ...savedEntries.map(
                (entry) => Padding(
            ...
            if (restoringHistory)
              ...
              const SectionCard(
                ...
                subtitle:
                    'Generate an outline, start a render job or conversion job to create records here.',
              )
            else
              ...entries.map(
                (entry) => Padding(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            'File `${entry.filename}` will be deleted from local app storage.',
          ),
          actions: [
            TextButton(
